...
$("#btn-delete-localstorage").addEventListener("click", () =>
...
const inputFields = {
  ...
  "#field-heuregeneration": {
    ...
    pattern: /\d{2}:\d{2}/g,
    defaultFn: () => {
      const [min_offset, max_offset] = [1, 6];
      const random_offset = Math.random() * max_offset + min_offset;
      const generationTime = new Date(
        new Date().getTime() - (HARDCODED_SORTIE_OFFSET + random_offset) * 60000
      );
...
(localStorage.getItem("reasons") || "").split("-").forEach((reason_value) => {
  $(`[name="field-reason"][value="${reason_value}"]`)
    ? ($(`[name="field-reason"][value="${reason_value}"]`).checked = true)
    : null;
});
